import { View, Text, TouchableOpacity, StyleSheet, TextInput, KeyboardAvoidingView, FlatList, Alert } from 'react-native';
...
const StudentChat = ({ closeModal, otherUser, staffId }) => {
    ...
    const chatId = [userId, staffId].sort().join('-');
    ...
    const sendMessage = () => {
      ...
      if (content.trim().length === 0) {
        ...
        Alert.alert('Message cannot be sent', 'Account not found or has been deleted.')
        return;
      }
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.nameView}>
          <Text style={styles.name}>{otherUser?.name || "Deleted Account"}</Text>
